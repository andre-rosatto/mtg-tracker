...
export default function ColorPicker({color='#fff', className, onColorChange, ...props}: ColorPickerProps) {
  return (
    <input
        ...
        className='w-10 h-10 p-0 border-none rounded-sm bg-transparent cursor-pointer appearance-none [&::-webkit-color-swatch-wrapper]:p-0 [&::-webkit-color-swatch]:border-none'
        {...props}
      />
  );
}